...
bool QGLESWIDGET::init_QGW(std::vector<QString> fileName)
{
    ...
    name = fileName[1];
    _textureDetails = loadTexture(name.toLatin1().data());
    name = fileName[2];
    _tex1 = loadTexture(name.toLatin1().data());
    name = fileName[3];
    _texNor = loadTexture(name.toLatin1().data());
...
void QGLESWIDGET::displayNormal()
{
    ...
    _shaderBill.begin();
    {
        ...
        glUniform3f(_shaderBill._lightDir,_camera._eye.x,_camera._eye.y,_camera._eye.z);
        ...
        glVertexAttribPointer(_shaderBill._biTagenl,3,GL_FLOAT,false,sizeof(QDataBoxOpengl::NORMALMAP),&dataNor._dataNor[0].bx);
